...
import { type CreateEventPayload, type GitHubEvent, type IssuesEventPayload, type PushEventPayload } from "./types.js"
 ...
 */
async function formatEvent(event: GitHubEvent, plainText: boolean): Promise<string> {
  const repo: string = event.repo.name

  const bullet: string = plainText ? "-" : chalk.red("-")
  ...
  switch (event.type) {
    case "PushEvent": {
      ...
      const commitCount: number = await getCommitCount(event, payload)
