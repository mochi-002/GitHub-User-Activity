...
async function fetchUserEvents(username: string): Promise<GitHubEvent> {
  try {
    const URL: string = `${GITHUB_API_BASE}/users/${username}/events`
    ...
    throw new GitHubApiError(
      "Failed to fetch user events from GitHub."
    )
  }
}
 ...
 */
async function getCommitCount(event: GitHubEvent, payload: PushEventPayload): Promise<number> {
  ...
  if (!response.ok) {
    throw new GitHubApiError(
      `Failed to compare commits: ${response.status}`,
      response.status
    )
  }

  const comparison: any = await response.json()
